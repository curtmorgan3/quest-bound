...
- **Jump to character:** Implemented in `src/pages/campaigns/campaign-controls/jump-to-character.tsx`. The component pulls from campaign context (`useCampaignContext`): `campaignPlayerCharacters`, `jumpToCharacter`, `viewingLocationId`. It computes `hasPlayerCharactersElsewhere` and only renders the trigger button when true. It owns the dialog open state and renders a small dialog listing player characters (avatar + name). On character click, calls `jumpToCharacter(characterId)` and closes the dialog. Prefer context over props; no props required.
...
- **Tile menu (Move Character):** Implemented in `src/pages/campaigns/campaign-controls/tile-menu.tsx`. The component pulls from campaign context (`useCampaignContext`): `viewingLocationId`, `selectedCharacters`, `moveSelectedCharactersTo`. It owns the menu open state and uses a render prop to expose `onTileMenuRequest(payload)` so the parent can pass it to LocationViewer. When the user clicks a tile in play mode, LocationViewer calls `onTileMenuRequest`; TileMenu opens the popover at `clientX/clientY`. On "Move Character" click, TileMenu calls `moveSelectedCharactersTo(viewingLocationId, tileId)` from context and closes the menu. Prefer context over props.
