...
import { useActiveRuleset, useItemCustomProperties } from '@/lib/compass-api';
import type { CustomPropertyType, Item } from '@/types';
...
function getEffectiveDefault(
  icpDefault: string | number | boolean | undefined,
  cpDefault: string | number | boolean | undefined,
  type: CustomPropertyType,
): string | number | boolean {
  if (icpDefault !== undefined) return icpDefault;
  if (cpDefault !== undefined) return cpDefault;
  return type === 'number' ? 0 : type === 'boolean' ? false : type === 'color' ? '' : '';
}

interface ManageItemCustomPropertiesPanelProps {
  item: Item;
  trigger?: React.ReactNode;
}

export function ManageItemCustomPropertiesPanel({
  item,
  trigger,
}: ManageItemCustomPropertiesPanelProps) {
  ...
  } = useItemCustomProperties(item.id);
  ...
  return (
    <>
      <Sheet>
        ...
        <SheetContent side='right' className='flex w-full flex-col gap-0 sm:max-w-md'>
          <SheetHeader className='shrink-0 border-b px-6 py-4'>
            <SheetTitle>Custom properties — {item.title}</SheetTitle>
            <SheetDescription>
              Custom properties are applied to all items of this type when added to an inventory.
            </SheetDescription>
          </SheetHeader>
          <div className='flex min-h-0 flex-1 flex-col gap-2 overflow-auto p-6'>
            ...
            {customProperties.length === 0 ? (
              <p className='text-sm text-muted-foreground py-4'>
                No custom properties assigned. Add one to define values when this item is added to
                an inventory.
              </p>
            ) : (
              <div className='flex min-h-0 flex-1 flex-col gap-2 overflow-auto'>
                {filteredProperties.length === 0 ? (
                  ...
                  filteredProperties.map((cp) => {
                    const icp = itemCustomProperties.find((r) => r.customPropertyId === cp.id);
                    const hasOverride = icp?.defaultValue !== undefined;
                    const effectiveDefault = icp
                      ? getEffectiveDefault(icp.defaultValue, cp.defaultValue, cp.type)
      ...
      <CustomPropertyPicker
        ...
        onSelect={async (id) => {
          await addItemCustomProperty(id);
        }}
      />
    </>
  );
