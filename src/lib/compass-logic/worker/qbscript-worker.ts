...
function cloneGameLogTimelineForPostMessage(timeline: ScriptGameLogEntry[]): ScriptGameLogEntry[] {
  return timeline.map((e) =>
    e.kind === 'log' ? { kind: 'log', args: prepareForStructuredClone(e.args) } : { ...e },
  );
}
...
async function handleExecuteItemEvent(payload: {
  ...
  try {
    ...
    const batchTs = result.scriptLogBatchTimestamp;
    await persistScriptLogs(db, {
      rulesetId: item.rulesetId,
      ...
      context: 'item_event',
      campaignId: payload.campaignId ?? null,
      ...(batchTs !== undefined ? { batchTimestamp: batchTs, sequenceStart: 1 } : {}),
    });
