-- RPC: delete all cloud data for one ruleset for the current user (RLS + sync_is_allowed apply).
-- Returns storage paths so the client can remove objects from Supabase Storage buckets.
...
BEGIN
  ...
  WHERE user_id = uid AND table_name = 'rulesets' AND entity_id = p_ruleset_id;
...
REVOKE ALL ON FUNCTION public.delete_remote_ruleset(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.delete_remote_ruleset(text) TO authenticated;
