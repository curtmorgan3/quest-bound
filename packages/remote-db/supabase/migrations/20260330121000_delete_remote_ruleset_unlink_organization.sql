-- Phase 7: Deleting a ruleset from the cloud removes its organization link so members lose access
-- (same effect as admin unlink). owner_user_id on the link matches the deleting user (ruleset owner).
