...
import { useItems } from '@/lib/compass-api/hooks/rulesets/use-items';

type ListRow =
  | { type: 'category'; label: string; estimatedSize: number }
  | { type: 'entry'; entry: Item; estimatedSize: number };
    ...
    const filtered = searchLower
      ? items.filter(
          (item) =>
            ...
            (item.category ?? 'Uncategorized').toLowerCase().includes(searchLower),
        )
      : items;
